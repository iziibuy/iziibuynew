    ...
    public function createPayment(PaymentMethodAccess $paymentMethodAccess, Request $request)
    {
        ...
        try {
            $order = ExternalOrder::create([
                ...
                'taxValue' => $request->taxValue ?? '0',
                'taxTotal' => $request->taxTotal ?? '0',
                'orderId' => $request->orderId,
                'description' => $request->description,
                'source_url' => $api->domain ?? '',
                'success_redirect_url' => $api->success_redirect_url ?? $api->domain ?? '',
                'failed_redirect_url' => $api->failed_redirect_url ?? $api->domain ?? '',
                'amount' => $request->amount,
                'currency' => $request->currency ?? 'NOK',
            ]);
        } catch (\Throwable $exception) {
            Log::error('Iziipay createPayment: could not create external order', [
                'payment_method_access_id' => $paymentMethodAccess->id,
                'message' => $exception->getMessage(),
            ]);

            return response()->json([
                'message' => 'Could not create payment order. Please contact support.',
                'status' => false,
            ], 500);
        }

        if ($paymentMethodAccess->paymentMethod == 'surfboard') {
            if (blank($paymentMethodAccess->surfboard_merchantId) || blank($paymentMethodAccess->surfboard_terminalId)) {
                return response()->json([
                    'message' => 'Surfboard terminal is not configured for this merchant. Please complete Surfboard setup in the dashboard.',
                    'status' => false,
                ], 400);
            }

...
class ElavonEnterpriseOnboardingHostedSubscription
{
    ...
    protected function shopperPhone(): string
    {
        ...
        return (string) ($addr->contact_number ?? $this->onboarding->user?->phone ?? '00000000');
    }
    ...
    protected function resolveShopperReferenceForSession(): ?string
    {
        if (filled($this->onboarding->shopperId)) {
            return $this->convergeResourceUrl(Endpoint::SHOPPER, (string) $this->onboarding->shopperId);
        }

        $shopper = $this->elavon->createShopper($this->getShopperCreateBody());
        if (! $shopper->isSuccess()) {
            Log::warning('Elavon enterprise onboarding HPP: could not create shopper', [
        ...
        if ($shopperId) {
            $this->onboarding->shopperId = $shopperId;
    ...
    protected function makeOrderCreateBody(float $amountNok, bool $vaultOnly = false): array
    {
        $description = $vaultOnly
            ? sprintf('Card registration — %s', $this->onboarding->company_name ?? $this->onboarding->id)
            : sprintf('Enterprise subscription — %s', $this->onboarding->company_name ?? $this->onboarding->id);
    ...
    public function getPaymentLink(float $amountNok, string $returnUrl, string $cancelUrl): array
    {
        ...
        if ($payment_session_create_response->isSuccess()) {
            ...
            Log::info('Elavon enterprise onboarding HPP: created payment session', [
                'enterprise_onboarding_id' => $this->onboarding->id,
    ...
    protected function makeTransactionCreateBody(PaymentSessionResponse $session, float $amountNok, ?string $storedCardId): array
    {
        $body = [
            ...
            'shopperReference' => (string) $this->onboarding->id,
            'description' => sprintf('Enterprise subscription — %s', $this->onboarding->company_name ?? $this->onboarding->id),

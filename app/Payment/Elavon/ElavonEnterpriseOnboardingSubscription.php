...
class ElavonEnterpriseOnboardingSubscription
{
    protected ElavonEnterpriseOnboardingHostedSubscription $hosted;

    protected Converge2 $elavon;

    protected string $apiBase;

    public function __construct(protected EnterpriseOnboarding $onboarding)
    {
        $this->onboarding->loadMissing('user');
        $this->hosted = new ElavonEnterpriseOnboardingHostedSubscription($onboarding);
    ...
    public function finalizeHostedSubscriptionFromSession(string $sessionId, Subscription $subscription, float $amountNok): array
    {
        ...
        if ($cardId === '') {
            Log::warning('Elavon enterprise onboarding: card vault failed after HPP return', [
                'enterprise_onboarding_id' => $this->onboarding->id,
        ...
        if ($primaryShopper !== '') {
            $this->onboarding->shopperId = $this->convergeEntityIdFromHrefOrId($primaryShopper) ?: $primaryShopper;
        }

        $this->onboarding->subscriptionMethod = 'elavon';
        $this->onboarding->status = true;
        $this->onboarding->last_paid_at = now();
            ...
            $subscription->charges()->create([
                ...
                'payment_details' => json_encode([
                    ...
                    'enterprise_onboarding' => [
                        ...
                        'company' => $this->onboarding->company_name,
                        'domain' => $this->onboarding->company_domain,
                    ],
                ]),
            ]);
        }
    ...
    public function getBillingStatus(): array
    {
        return [
            'provider' => 'elavon',
            'company' => $this->onboarding->company_name,
            'active' => (bool) $this->onboarding->status,
            'stored_card_id' => $this->onboarding->subscription?->key,
            'shopper_id' => $this->onboarding->shopperId,
            'last_paid_at' => $this->onboarding->last_paid_at,
            'subscription' => $this->onboarding->subscription,
        ];
    }
    ...
    {
        $email = $this->onboardingShopperEmail();
        $addr = $this->onboarding->company_address;
        ...
        $body = [
            ...
            'doCapture' => 1,
            'shipTo' => [
                'fullName' => trim(($user?->name ?? '').' '.($user?->last_name ?? '')) ?: ($this->onboarding->company_name ?? 'Enterprise'),
                'company' => $this->onboarding->company_name ?? '',
            ...
            'shopperReference' => (string) $this->onboarding->id,
            'shopperStatement' => [
    ...
    {
        return (string) ($this->onboarding->company_email ?: $this->onboarding->user?->email ?: 'external@localhost');
    }
    ...
    protected function shopperReferenceCandidatesFromPaymentSession(PaymentSessionResponse $session): array
    {
        ...
        if (filled($this->onboarding->shopperId)) {
            $candidates[] = (string) $this->onboarding->shopperId;
        }
    ...
    protected function shopperReferenceCandidatesFromNewShopperRecord(): array
    {
        ...
        $shopper = $this->elavon->createShopper([
            'fullName' => $this->onboarding->company_name ?? 'Enterprise',
            'company' => $this->onboarding->company_name ?? '',
            'primaryAddress' => [
                'street1' => (string) ($addr->street ?? 'N/A'),
                'street2' => '',
            ...
            'primaryPhone' => (string) ($addr->contact_number ?? $this->onboarding->user?->phone ?? '00000000'),
            'email' => $this->onboardingShopperEmail(),
        ]);
